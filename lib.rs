/// The main goal off this project is to create a text wrapper with user specific interactions with Ctrl-C input.

/// lib.rs has the main processes of the program. It begins by using some needed packages, then begins setting up signals that would let the program know when Ctrl-C is used.
/// It then sets up a platform. 
/// Then, it creates the signal handler for Ctrl-C, giving it an input timer of 100 ms.
/// Finally, it creates another signal handler, but this time with a costum input timer.



// Copyleft (c) 2015 CtrlC developers
